...
__constant__ box con_box[N];
//__constant__ bool con_res[N];
...
int main()
{
    ...
    HANDLE_ERROR(cudaMemcpyToSymbol(con_box,&b,sizeof(box)*N));

	NMS_GPU3<<<dim3(1,count,1),count>>>(d_res);
	...
	printf("GPUִ executed Time: %.5f (ms)\n", gpu_time);
